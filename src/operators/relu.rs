...
pub struct ReluOperator<'a> {
    // Relu specific fields for the relu operator if needed
    base_data: BaseData<'a>,
}

impl<'a> BaseOperator<'a> for ReluOperator<'a> {
    fn new(
    ...
    fn generate_cpp_code(&mut self) -> Result<Vec<String>> {
        let [input] = self.base_data().inputs.as_slice() else {
            return Err(color_eyre::eyre::eyre!(
                "Relu operator requires exactly 1 input"
            ));
        };
        ...
        let [dim0, dim1] = shape.as_slice() else {
            return Err(color_eyre::eyre::eyre!("Relu operator requires 2D input"));
        };
        self.set_output_var_name(out_var.clone())?;
        Ok(vec![
            format!(
                "    const Matrix<{r}, {c}> {out_var} = relu<{r}, {c}>({in_var});",
                r = dim0,
                c = dim1
            )
            .to_string(),
