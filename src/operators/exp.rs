...
pub struct ExpOperator<'a> {
    // Exp specific fields for the exp operator if needed
    base_data: BaseData<'a>,
}

impl<'a> BaseOperator<'a> for ExpOperator<'a> {
    fn new(
    ...
    fn generate_cpp_code(&mut self) -> Result<Vec<String>> {
        ...
        let [shape0, shape1] = shape.as_slice() else {
            return Err(color_eyre::eyre::eyre!(
                "Input tensor must be 2D for Exp operator"
            ));
        };

        self.set_output_var_name(out_var.clone())?;
        Ok(vec![format!(
            "    const Matrix<{shape0}, {shape1}> {out_var} = exp<{shape0}, {shape1}>({in_var});"
        )])
    }
}
